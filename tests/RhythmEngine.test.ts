...
test("uses osu mania v2 head window boundaries", () => {
  const early = new ManiaRulesEngine(createChart([{ column: 1, absolute_time: 1, weight: 0 }]));
  early.press(0, 0.864);
  assert.equal(early.note_states[0], NoteState.Passed);

  const late = new ManiaRulesEngine(createChart([{ column: 1, absolute_time: 1, weight: 0 }]));
  late.press(0, 1.112);
  assert.equal(late.note_states[0], NoteState.Passed);

  const early_miss = new ManiaRulesEngine(createChart([{ column: 1, absolute_time: 1, weight: 0 }]));
...
test("keeps too-early presses clear and emits the native no-op event", () => {
  const engine = new ManiaRulesEngine(createChart([{ column: 1, absolute_time: 1, weight: 0 }]));
...
test("earliest and nearest registration choose different overlapping notes", () => {
  ...
  const earliest = new ManiaRulesEngine(chart, "earliest");
  assert.equal(earliest.press(0, 1.11), 0);
  assert.equal(earliest.note_states[0], NoteState.Passed);

  const nearest = new ManiaRulesEngine(chart, "nearest");
...
test("positions hold endpoints independently across scroll changes", () => {
  const engine = new ManiaRulesEngine({
    ...createChart([
      { column: 1, absolute_time: 1, weight: 1 },
      { column: 1, absolute_time: 2, weight: -1 },
    ]),
...
test("constant scroll positions notes by absolute time", () => {
  ...
  const regular = new ManiaRulesEngine(chart);
  regular.update(1, 2, 2);
  assertClose(regular.visible_notes[0]?.start_dt, 1.5);

  const constant = new ManiaRulesEngine(chart, "earliest", 1, true);
...
test("keeps hit windows constant in real time at changed music rates", () => {
  const fast = new ManiaRulesEngine(createChart([{ column: 1, absolute_time: 1, weight: 0 }]), "earliest", 2);
  fast.press(0, 1 + 0.112 * 2);
  assert.equal(fast.note_states[0], NoteState.Passed);
  assertClose(fast.logic_events[0]?.delta_time, 0.112);

  const slow = new ManiaRulesEngine(createChart([{ column: 1, absolute_time: 1, weight: 0 }]), "earliest", 0.5);
...
test("scales miss deadlines into chart time at changed music rates", () => {
  const engine = new ManiaRulesEngine(createChart([{ column: 1, absolute_time: 1, weight: 0 }]), "earliest", 2);
