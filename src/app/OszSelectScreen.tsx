...
export function OszSelectScreen({ archive, importing, import_error, onBack, onImport, onPlay }: OszSelectScreenProps) {
  ...
  const play = () => onPlay(selected_chart, loadInputBindings(inputLayout(selected_chart)), archive.song);
  ...
  return (
    <main className={`osz-select-screen${dragging ? " dragging" : ""}`} onDragEnter={(event) => { event.preventDefault(); setDragging(true); }}
      ...
      <header className="osz-header">
        ...
        <span><FileArchive /> {archive.file_name}</span>
      ...
      <section className="osz-heading">
        <h1>{archive.song.title}</h1>
        <p>{archive.song.artist}</p>
        <small>{archive.song.charts.length} playable {archive.song.charts.length === 1 ? "chart" : "charts"}</small>
      </section>
      ...
      <footer className="osz-footer">
        <div><span>SELECTED DIFFICULTY</span><strong>{selected_chart.name}</strong>{import_error && <p>{import_error}</p>}</div>
        <button type="button" onClick={play}><Play fill="currentColor" /> Play chart</button>
      </footer>
